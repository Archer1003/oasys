...
public class UserController {
	...
	public String usermanage(HttpServletRequest req,Model model,@RequestParam(value="page",defaultValue="0") int page,
		...
		Page<User> userspage = udao.findByIsLock(0,user.getDept().getSysId(), pa);
	...
	public String userPaging(HttpServletRequest req,Model model,@RequestParam(value="page",defaultValue="0") int page,
			@RequestParam(value="size",defaultValue="10") int size,
		...
		if(StringUtil.isEmpty(usersearch)){
			userspage =  udao.findByIsLock(0,user.getDept().getSysId(), pa);
		}else{
			System.out.println(usersearch);
			userspage = udao.findnamelike(usersearch,user.getDept().getSysId(), pa);
		}
		List<User> users=userspage.getContent();
		model.addAttribute("users",users);
		model.addAttribute("page", userspage);
	...
	public String usereditget(HttpServletRequest req, @RequestParam(value = "userid",required=false) Long userid, Model model) {
		if(userid!=null){
			User user = udao.findOne(userid);
	...
	public String usereditpost(HttpServletRequest req,User user,
		...
		Long userId = Long.parseLong(session.getAttribute("userId") + "");
		User tuser=udao.findOne(userId);
